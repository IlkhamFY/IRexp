...
licence segregation, and validation artefacts --- not elucidation accuracy.
...
\begin{itemize}
  ...
  \item Harvest / pipeline code: \url{https://github.com/IlkhamFY/spectro-agent}
        (Section~\ref{sec:codeavail})
  \item Zenodo archival snapshot: DOI to be minted with the PI
        (data-only deposit; commercial pool primary --- not the combined
        IRSpectra-Bench research archive). Pending; no DOI is claimed here.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Attribution.} Cite this Data Descriptor and the Zenodo DOI (when
        minted) and attribute originating articles through each record's
        \texttt{source\_doi}.
\end{itemize}
...
\begin{itemize}
  ...
  \item Manuscript, frozen count manifests, and figures:
        \url{https://github.com/IlkhamFY/IRexp}
        (no bulk JSONL under \texttt{data/} in that repository)
  \item Harvest / pipeline source: \url{https://github.com/IlkhamFY/spectro-agent}
        (Section~\ref{sec:codeavail})
  \item Zenodo: archival DOI pending --- to be minted with the PI for a
        \emph{data-only} deposit (primary artifact = commercial pool,
        \texttt{cc-by-4.0} metadata + SA companion).
        Do not reuse the combined IRSpectra-Bench research \texttt{.zenodo.json}
        as the Sci Data archival record. No DOI is invented here.
\end{itemize}
...
A release tag matching this Descriptor will be pinned on spectro-agent when the
Zenodo archive is minted (placeholder: \texttt{irexp-scidata}; record the commit
SHA at deposit). Until then, \texttt{main} at
\texttt{6c18914} (2026-08-31) is the public snapshot corresponding to the
harvest/validation scripts cited here.
...
Council of Canada (NSERC), funding reference number 596133-2025 (CREATE for
Accelerated Discovery, AccelD), delivered through the Acceleration Consortium.
