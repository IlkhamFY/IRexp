...
pool~\citep{hrynaszkiewicz2012open}; NC* are held aside; remaining empty/unknown rows
are not redistributed --- aligning redistributable intent with the
...
pools as shown.
The public Hugging Face upload and the Zenodo primary file are the commercial
dataset of record (archival version DOI \url{https://doi.org/10.5281/zenodo.22822285}).
...
quadruples.
That total is not part of the public dataset-of-record upload.
The public dataset of record (DoR) is the commercial pool only:
\texttt{irexp\_commercial.jsonl.gz} (88,545 CC-BY/CC0 records), on Hugging Face
...
stamped.
The same Hub upload includes \texttt{irexp\_resolved\_commercial.jsonl.gz}
(28,899) and \texttt{train\_no\_bench\_commercial.jsonl.gz} (28,753).
ShareAlike (1,897; Chemotion plus rare PMC ShareAlike; source licence
CC-BY-SA-4.0), non-commercial (21,823; NC*), ND (5; CC-BY-ND), and
empty/unknown (8,963) are licence-pool counts in the research corpus.
They are retained for research and are not part of the public Hugging Face
upload.
...
The public Hugging Face upload (\texttt{ilkhamfy/IRexp}, under \texttt{data/})
is the commercial DoR and its two commercial companions (Access).
The spectro-agent working tree keeps the research corpus and pool files under
...
The public Hugging Face upload is the commercial dataset of record and its two
commercial companions.
Other rows are research-corpus counts and are not part of that upload.}\label{tab:files}
\scriptsize
...
\midrule
\texttt{irexp\_commercial.jsonl.gz} & 88,545 & Public Hub DoR and Zenodo primary; CC-BY-4.0 packaging \\
\texttt{irexp\_resolved\_commercial.jsonl.gz} & 28,899 & Public Hub; structure-linked commercial subset \\
\texttt{train\_no\_bench\_commercial.jsonl.gz} & 28,753 & Public Hub; commercial subset, benchmark InChIKeys held out \\
\midrule
\texttt{irexp.jsonl.gz} & 121,233 & Research total; not in the public Hub upload \\
\texttt{irexp\_resolved.jsonl.gz} & 57,646 & Research split (multi-licence); not in the public Hub upload \\
\texttt{train\_no\_bench.jsonl.gz} & 42,808 & Research split; benchmark InChIKeys held out; not in the public Hub upload \\
\texttt{train\_no\_bench\_nmr.jsonl.gz} & 32,949 & Same research split with both $^{1}$H and $^{13}$C; not in the public Hub upload \\
\texttt{pretrain\_ir.jsonl.gz} & 119,345 & PMC-only research pool; not in the public Hub upload \\
\texttt{irexp\_sharealike.jsonl.gz} & 1,897 & Corpus pool, CC-BY-SA-4.0; not in the public Hub upload \\
\texttt{irexp\_non\_commercial.jsonl.gz} & 21,823 & Corpus pool, NC*; not in the public Hub upload \\
\texttt{irexp\_other.jsonl.gz} & 5 & Corpus pool, CC-BY-ND; not in the public Hub upload \\
\texttt{irexp\_empty\_unknown.jsonl.gz} & 8,963 & Corpus count; not in the public Hub upload \\
\bottomrule
\end{tabular}
\end{table}
...
2026-08-26/27), the pools in Table~\ref{tab:files} sum to the research-corpus
total (Figure~\ref{fig:distribution}b).
The public upload is the commercial pool, packaged under CC-BY-4.0.
ShareAlike, non-commercial, ND, and empty/unknown remain in that sum.
They are retained for research and are not part of the public Hub upload.
...
Harvest provenance, not part of the public Hub upload:
...
\begin{itemize}
  \item Public dataset of record, Hugging Face
        ...
        \begin{itemize}
          \item \texttt{irexp\_commercial.jsonl.gz} (88,545): CC-BY-4.0 packaging,
                per-record CC-BY/CC0 stamps.
                This commercial DoR is also the Zenodo primary file
                \url{https://doi.org/10.5281/zenodo.22822285}.
          \item \texttt{irexp\_resolved\_commercial.jsonl.gz} (28,899):
                structure-linked commercial subset.
          \item \texttt{train\_no\_bench\_commercial.jsonl.gz} (28,753):
                commercial subset with benchmark InChIKeys held out.
        \end{itemize}
  \item Retained for research, not part of the public DoR upload:
        the multi-licence total, ShareAlike (1,897), non-commercial (21,823),
        ND (5), and empty/unknown (8,963).
...
\begin{itemize}
  ...
  \item \textbf{Licence filter.} Public redistribution is the commercial dataset
        of record only (\texttt{irexp\_commercial.jsonl.gz}; CC-BY-4.0 packaging;
        per-record CC-BY/CC0 stamps; Section~\ref{sec:datarecords}).
        ShareAlike (1,897; source licence CC-BY-SA-4.0), non-commercial
        (21,823; NC*), and ND (5; CC-BY-ND) are corpus pools retained for
        research.
        They are not in the public Hugging Face upload.
        Do not relicence Chemotion/ShareAlike rows as CC-BY.
        The multi-licence total and the empty/unknown rows are not part of the
        public DoR upload.
  ...
  \item \textbf{Training without benchmark leakage.} If holding out structures used
        ...
        Commercial training uses the commercial companion on the Hub revision
  ...
  \item \textbf{Structure coverage.} Supervised structure tasks need rows with
        ...
        Use the structure-linked commercial subset on the public Hub upload
        (Section~\ref{sec:datarecords}).
  \item \textbf{FAIR reuse example.} For a redistributable training set, load the
        commercial DoR; drop quarantine IDs; keep \texttt{source\_doi} /
        \texttt{pmcid} for attribution; prefer structure-linked commercial rows
        when SMILES is required.
        Chemotion/ShareAlike rows in the research corpus may be remixed only
        under CC-BY-SA-4.0; that pool is not in the public Hub upload.
...
\begin{itemize}
  ...
  \item \textbf{Licence mix.} The research corpus is multi-licence.
        Public redistribution is the commercial dataset of record only
        (Section~\ref{sec:datarecords}).
        ShareAlike, NC*, ND, and empty/unknown rows are retained for research
        and are not part of that upload.
...
(Section~\ref{sec:datarecords}).
That Hub revision ships this commercial deposit with
\texttt{ir\_shared\_in\_paper} and \texttt{ir\_table\_flatten\_suspect}
retained on the commercial DoR only (rows not dropped), plus companion configs
...
Section~\ref{sec:datarecords} and is not part of the public DoR upload.
ShareAlike, non-commercial, ND, and empty/unknown rows are retained for
research and are not in that Hugging Face upload.
...
\begin{itemize}
  ...
  \item \textbf{Harvest/pipeline source}: \url{https://github.com/IlkhamFY/spectro-agent}
   ...
   \item \textbf{License:}
    \begin{itemize}
          \item Commercial DoR (Hugging Face commercial configuration and the Zenodo primary file): CC-BY-4.0 packaging, with per-record source licences still stamped.
          \item Chemotion rows in the research corpus (1,888): CC-BY-SA-4.0~\citep{chemotion2024}.
                The ShareAlike pool (1,897) is not in the public Hub upload.
          \item Non-commercial (21,823; NC*) and ND (5; CC-BY-ND) are corpus pools, not part of the public Hub upload.
          \item PMC-sourced rows are mixed Creative Commons, stamped per article (\texttt{docs/LICENCE\_REMEDIATION.md}).
                Empty/unknown rows (8,963) and the multi-licence total are not part of the public DoR upload.
