...
Public redistribution is limited to stamped-licence pools.
The commercial dataset of record is the CC-BY/CC0 pool of 88,545 records,
...
Corpus size and the public stamped-licence pools are in
...
Public redistribution is the stamped-licence pools; grouped bars show
...
text-mined band lists (public redistribution: stamped-licence pools).
...
release is the stamped-licence pools in Section~\ref{sec:datarecords}.
SDBS is a view-only archive of digitised absorbance spectra
($\sim$54,000 FT-IR entries).
Those pools are redistributable text-mined band lists.
...
modelling on structure-linked rows from a stamped-licence pool, and serving as
...
Public redistribution is the stamped-licence pools in Table~\ref{tab:files}.
Commercial reuse must use \texttt{license\_pool == "commercial"}.
...
quadruples.
That total is not a public redistribution file.
Public redistribution is limited to the stamped-licence pools in
Table~\ref{tab:files}.
The commercial dataset of record (DoR) is \texttt{irexp\_commercial.jsonl.gz}
(88,545 CC-BY/CC0 records), on Hugging Face and as the Zenodo primary file
(\url{https://doi.org/10.5281/zenodo.22822285}).
That deposit is packaged under CC-BY-4.0, with per-record source licences still
stamped.
\texttt{irexp\_sharealike.jsonl.gz} (1,897; Chemotion plus rare PMC ShareAlike)
is packaged under CC-BY-SA-4.0.
\texttt{irexp\_non\_commercial.jsonl.gz} (21,823) is source-stamped NC*; its
packaging is research redistribution under those terms.
\texttt{irexp\_other.jsonl.gz} (5) is source-stamped CC-BY-ND; its packaging is
research redistribution under those terms.
Empty/unknown rows (8,963) remain in the research-corpus counts and are not
redistributed (removed from the public Hugging Face redistribution).
...
Public redistributable files are on Hugging Face (\texttt{ilkhamfy/IRexp};
stamped-licence pools under \texttt{data/}; Access).
...
Public redistributable files are the stamped-licence pools.
The multi-licence research total and the empty/unknown count are not public
redistribution files.
Names follow the Hugging Face layout (\texttt{data/});
...
\midrule
\texttt{irexp.jsonl.gz} & 121,233 & Multi-licence research total; not redistributed \\
\texttt{irexp\_resolved.jsonl.gz} & 57,646 & Structure-linked research split (multi-licence) \\
\texttt{train\_no\_bench.jsonl.gz} & 42,808 & Research split (multi-licence); benchmark InChIKeys held out \\
\texttt{train\_no\_bench\_nmr.jsonl.gz} & 32,949 & Same research split with both $^{1}$H and $^{13}$C \\
\texttt{pretrain\_ir.jsonl.gz} & 119,345 & PMC-only IR pretrain pool (multi-licence) \\
\midrule
\texttt{irexp\_commercial.jsonl.gz} & 88,545 & Public; CC-BY-4.0 packaging; CC-BY/CC0 stamps \\
\texttt{irexp\_sharealike.jsonl.gz} & 1,897 & Public; CC-BY-SA-4.0 (Chemotion + rare PMC SA) \\
\texttt{irexp\_non\_commercial.jsonl.gz} & 21,823 & Public; source-stamped NC*; redistribution under those terms \\
\texttt{irexp\_other.jsonl.gz} & 5 & Public; source-stamped CC-BY-ND; redistribution under those terms \\
\texttt{irexp\_empty\_unknown.jsonl.gz} & 8,963 & Research-corpus count; not redistributed \\
\bottomrule
\end{tabular}
\end{table}
...
Packaging is in the release-scope paragraph.
Empty/unknown rows remain in that sum and are not redistributed.
...
\begin{itemize}
  \item Public redistributable files, Hugging Face
        ...
        \begin{itemize}
          \item \texttt{irexp\_commercial.jsonl.gz} (88,545): CC-BY-4.0 packaging,
                ...
                \url{https://doi.org/10.5281/zenodo.22822285}.
          \item \texttt{irexp\_sharealike.jsonl.gz} (1,897): CC-BY-SA-4.0.
          \item \texttt{irexp\_non\_commercial.jsonl.gz} (21,823): source-stamped
                NC*; packaging is research redistribution under those terms.
          \item \texttt{irexp\_other.jsonl.gz} (5): source-stamped CC-BY-ND;
                packaging is research redistribution under those terms.
        \end{itemize}
  \item Not redistributed: the multi-licence research total and the empty/unknown
        rows (counts in Table~\ref{tab:files}), removed from the public Hugging
        Face redistribution.
...
\begin{itemize}
  ...
  \item \textbf{Licence filter.} Public redistribution is the stamped-licence pools
        in Section~\ref{sec:datarecords}.
        Commercial reuse uses \texttt{irexp\_commercial.jsonl.gz} (CC-BY-4.0
        packaging; per-record CC-BY/CC0 stamps).
        ShareAlike reuse uses \texttt{irexp\_sharealike.jsonl.gz} (CC-BY-SA-4.0);
        do not relicence those rows as CC-BY.
        \texttt{irexp\_non\_commercial.jsonl.gz} is source-stamped NC*, and
        \texttt{irexp\_other.jsonl.gz} is source-stamped CC-BY-ND; each is
        packaged as research redistribution under those terms.
        The multi-licence research total and the empty/unknown rows are not
        redistributed.
  ...
  \item \textbf{Structure coverage.} Supervised structure tasks need rows with
        ...
        Use a structure-linked subset of a stamped-licence pool
        (Section~\ref{sec:datarecords}).
  \item \textbf{FAIR reuse example.} For a redistributable training set, load the
        ...
        ShareAlike rows may be remixed only under CC-BY-SA-4.0
        (Section~\ref{sec:datarecords}).
...
\begin{itemize}
  ...
  \item \textbf{Licence mix.} The research corpus is multi-licence.
        Public redistribution is the stamped-licence pools in
        Section~\ref{sec:datarecords}.
...
Section~\ref{sec:datarecords} and is not a public redistribution file.
ShareAlike, non-commercial, and ND files are the other public redistributable
pools (packaging in that section).
Empty/unknown rows are not redistributed.
  ...
  \item \textbf{Harvest/pipeline source}: \url{https://github.com/IlkhamFY/spectro-agent}
   ...
   \item \textbf{License:}
    \begin{itemize}
          ...
          \item ShareAlike file: CC-BY-SA-4.0.
                Chemotion (1,888): CC-BY-SA-4.0~\citep{chemotion2024}.
                These rows are not in the commercial DoR.
          \item Non-commercial file (21,823): source-stamped NC*; packaging is research redistribution under those terms.
          \item ND file (\texttt{irexp\_other.jsonl.gz}, 5): source-stamped CC-BY-ND; packaging is research redistribution under those terms.
