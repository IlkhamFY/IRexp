...
%% sn-jnl defines \orcid as an immediate \href to Orcidlogo.eps. Called at
%% this point in the source, that ships the logo above the title. Append the
%% same linked logo to the author line when \orcid follows \author / \email.
\makeatletter
\renewcommand{\orcid}[1]{\g@addto@macro\artauthors{\,\href{#1}{\orcidlogo}}}
\makeatother
\author*[1]{\fnm{Ilkham} \sur{Yabbarov}}\email{yabbaroi@mcmaster.ca}\orcid{https://orcid.org/0009-0004-9393-9822}
\author[1]{\fnm{Rudra} \sur{Sondhi}}\orcid{https://orcid.org/0009-0003-3034-7347}
\author*[1,2,3]{\fnm{Rodrigo A.} \sur{Vargas-Hern{\'a}ndez}}\email{vargashr@mcmaster.ca}\orcid{https://orcid.org/0000-0002-5559-6521}
...
\abstract{%
IRexp is a collection of experimental infrared band lists
...
The multi-licence research corpus holds 121,233 records (119,345 PMC Open Access
...
Each record carries a source identifier and a stamped source licence.
The public dataset of record is the commercial CC-BY/CC0 pool of 88,545 records,
packaged under CC-BY-4.0.
ShareAlike records, including the Chemotion rows, and non-commercial records
are separate Hub companions under their source licences and are excluded from
that commercial pool.
Intended reuse is multimodal training, retrieval, and tool input.
...
and a stratified expert human band-list audit of 161 records on the
commercial dataset of record.
...
IRexp is a collection of experimental IR band lists
...
Each record carries a source identifier and a stamped licence
pool~\citep{hrynaszkiewicz2012open,carroll2011openaccess}.
Corpus size and the public commercial dataset of record are in
Section~\ref{sec:datarecords}.
...
format.
NMRTrans reports experimental NMR spectra and the NMRSpec
corpus~\citep{yang2026nmrtrans}.
...
\textbf{IRexp} holds 121,233 experimental band lists (cm$^{-1}$ positions) in the
multi-licence research corpus, with per-record licence pools.
The public dataset of record is the commercial CC-BY/CC0 subset (88,545);
grouped bars also show structure-linked records (57,646).
...
text-mined band lists (public redistribution: the commercial dataset of record).
...
redistributable numeric records with source-identifier attribution and explicit licence pools;
...
Among openly redistributable \emph{text-derived IR band lists}, IRexp's public
release is the commercial dataset of record in Section~\ref{sec:datarecords}.
SDBS is a view-only archive of digitised absorbance spectra
($\sim$54,000 FT-IR entries).
That commercial pool is a redistributable collection of text-mined band lists.
...
Intended reuse includes multimodal pretraining and supervised IR$\to$structure
modelling on structure-linked commercial rows, and serving as
the literature substrate for complementary elucidation benchmarks if needed.
...
(\texttt{seen\_papers.txt.gz}, retained with the research corpus; not a Hub file).
...
Released \texttt{source\_doi} values are \texttt{PMC:*} accessions or the Chemotion
deposit DOI; no paywalled publisher DOI appears in the research corpus
(Section~\ref{sec:datarecords}).
...
links were ignored.
Commercial-use (CC-BY/CC0) rows form the primary archival
pool~\citep{hrynaszkiewicz2012open}.
NC* rows are deposited as a separate Hub companion under their source licence
and are excluded from that pool.
Empty/unknown rows are not redistributed.
Licence assignment follows article-level terms rather than
...
\texttt{seen\_papers.txt.gz} (188,016 PMC identifiers), retained with the
research corpus and available on request.
...
That equality is a convenience for this row type
(Section~\ref{sec:datarecords}).
...
\begin{enumerate}
  ...
  \item Persist harvest provenance in \texttt{seen\_papers.txt.gz} and optional
        ...
        intermediate prose fields; the curated research corpus is in
        Section~\ref{sec:datarecords}).
\end{enumerate}
...
Europe PMC licence join with Crossref recovery; segregation into licence
pools as shown.
The Hugging Face dataset of record and the Zenodo primary file are the commercial
pool (archival version DOI \url{https://doi.org/10.5281/zenodo.22822285}).
ShareAlike and non-commercial files on the same Hub repository are separate
licence-bound companions.
...
structure fields.
Structure coverage of the research corpus is 47.5\% (57,646 / 121,233).
The multi-licence structure-linked split is \texttt{irexp\_resolved}
(research corpus).
The public structure-linked commercial subset is
\texttt{data/irexp\_resolved\_commercial.jsonl.gz}.
...
The research corpus remains multi-licence.
The Zenodo primary file and the Hugging Face dataset of record are the commercial
pool (Table~\ref{tab:files}).
The same Hub repository also carries ShareAlike and non-commercial companions
under their source licences.
Those files are excluded from the commercial pool.
...
The multi-licence research corpus holds 121,233 records, of which 57,646 are
structure-linked and 39,118 are IR~$+$~$^{1}$H~$+$~$^{13}$C~$+$~structure
quadruples.
That total is construction and licence-pool accounting.
It is not deposited as a Hub file.
The public dataset of record (DoR) is the commercial pool only:
\texttt{data/irexp\_commercial.jsonl.gz} (88,545 CC-BY/CC0 records), on Hugging Face
(\texttt{ilkhamfy/IRexp}) and as the Zenodo primary file
(\url{https://doi.org/10.5281/zenodo.22822285}).
That deposit is packaged under CC-BY-4.0, with per-record source licences still
stamped.
The same Hub revision includes \texttt{data/irexp\_resolved\_commercial.jsonl.gz}
(28,899) and \texttt{data/train\_no\_bench\_commercial.jsonl.gz} (28,753),
both subsets of the commercial pool.
Two further files on that revision are licence-bound companions and do not
enter the CC-BY dataset of record:
\texttt{data/irexp\_sharealike.jsonl.gz} (1,897; Chemotion plus rare PMC
ShareAlike; source licence CC-BY-SA-4.0) and
\texttt{data/irexp\_non\_commercial.jsonl.gz} (21,823; NC*).
ND (5; CC-BY-ND) and empty/unknown (8,963) remain research-corpus counts and
are not Hub files.

\subsection{Object definition}

Each IRexp record is a JSON object; see Table~\ref{tab:fields}.
The \texttt{id} field is a unique stable internal record identifier.
InChIKey is stored in \texttt{inchikey} and is not unique across the corpus:
57,646 structure-linked records comprise 54,985 InChIKeys (about 2.7 thousand
extra rows repeat a molecule).
\texttt{ir\_shared\_in\_paper} and \texttt{ir\_table\_flatten\_suspect} exist on
the commercial DoR only (flag-only; rows not dropped).
\texttt{source\_doi} is a source identifier, not always a DOI.
...
\midrule
id & string & Unique stable internal record identifier \\
ir\_bands\_cm-1 & float list & Experimental IR peak positions (cm$^{-1}$) \\
ir\_source & string & \texttt{experimental} for all released rows \\
source\_doi & string & Source identifier: PMC accession (\texttt{PMC:\ldots}) or Chemotion deposit DOI \\
...
ir\_shared\_in\_paper & bool & Commercial DoR only: identical IR list shared by $\geq$2 records in one paper; flag-only \\
ir\_table\_flatten\_suspect & bool & Commercial DoR only: suspected table-flatten or column-misread list; flag-only \\
\bottomrule
\end{tabular}
\end{table}
...
Listing~\ref{lst:example} uses the InChIKey as \texttt{id} for that resolved
row, as do the Chemotion rows.
That is a convenience for those rows; the general \texttt{id} field remains a
unique internal identifier.
Rates for \texttt{ir\_shared\_in\_paper} and
\texttt{ir\_table\_flatten\_suspect} are in Technical Validation.
...
The public Hugging Face repository (\texttt{ilkhamfy/IRexp}, under \texttt{data/};
revision in the Access subsection) holds the commercial dataset of record,
two commercial subsets, and the ShareAlike and non-commercial companions
(Table~\ref{tab:files}).
The spectro-agent working tree keeps the research corpus, including the
multi-licence total, the ND pool, and the empty/unknown pool, under
\texttt{data/irexp/} and \texttt{data/irexp/licence\_pools/}.
Frozen counts are mirrored here as \texttt{data/irexp\_stats.json}.

\begin{table}[ht]
\caption{IRexp files and licence-pool counts.
The public Hugging Face repository (\texttt{data/}) holds the commercial dataset
of record, two commercial subsets, and licence-bound ShareAlike and
non-commercial companions.
Those companions are excluded from the CC-BY dataset of record.
Remaining rows are research-corpus counts and are not Hub files.}\label{tab:files}
\scriptsize
...
\midrule
\texttt{data/irexp\_commercial.jsonl.gz} & 88,545 & Public Hub dataset of record and Zenodo primary; CC-BY-4.0 packaging \\
\texttt{data/irexp\_resolved\_commercial.jsonl.gz} & 28,899 & Public Hub; structure-linked commercial subset \\
\texttt{data/train\_no\_bench\_commercial.jsonl.gz} & 28,753 & Public Hub; commercial subset, benchmark InChIKeys held out \\
\texttt{data/irexp\_sharealike.jsonl.gz} & 1,897 & Public Hub companion; CC-BY-SA-4.0 (Chemotion and rare PMC ShareAlike); excluded from the CC-BY dataset of record \\
\texttt{data/irexp\_non\_commercial.jsonl.gz} & 21,823 & Public Hub companion; NC*; excluded from the CC-BY dataset of record \\
\midrule
\texttt{irexp.jsonl.gz} & 121,233 & Research-corpus total; not a Hub file \\
\texttt{irexp\_resolved.jsonl.gz} & 57,646 & Research split (multi-licence); not a Hub file \\
\texttt{train\_no\_bench.jsonl.gz} & 42,808 & Research split; benchmark InChIKeys held out; not a Hub file \\
\texttt{train\_no\_bench\_nmr.jsonl.gz} & 32,949 & Same research split with both $^{1}$H and $^{13}$C; not a Hub file \\
\texttt{pretrain\_ir.jsonl.gz} & 119,345 & PMC-only research pool; not a Hub file \\
\texttt{irexp\_other.jsonl.gz} & 5 & Research-corpus pool, CC-BY-ND; not a Hub file \\
\texttt{irexp\_empty\_unknown.jsonl.gz} & 8,963 & Research-corpus count; not a Hub file \\
\bottomrule
\end{tabular}
\end{table}
...
Provenance of that corpus is 119,345 PMC and 1,888 Chemotion, from 15,416
unique PMC accessions (Figure~\ref{fig:distribution}a).
Further modality slices (Figure~\ref{fig:distribution}c): 87,075 records (72\%)
carry $^{1}$H and/or $^{13}$C NMR strings; 47,521 have structure plus any NMR.
Structure-linked and quadruple totals are in the release-scope paragraph.
...
2026-08-26/27), the licence pools in Table~\ref{tab:files}
(commercial, ShareAlike, non-commercial, ND, and empty/unknown) sum to the
research-corpus total (Figure~\ref{fig:distribution}b).
The public dataset of record is the commercial pool, packaged under CC-BY-4.0.
ShareAlike and non-commercial counts in that sum are also deposited as separate
Hub companions (Table~\ref{tab:files}).
ND and empty/unknown remain research-corpus counts and are not Hub files.
...
Harvest provenance is retained with the research corpus and is not a Hub deposit:
...
\begin{itemize}
  \item Public dataset of record, Hugging Face
        \url{https://huggingface.co/datasets/ilkhamfy/IRexp},
        revision \texttt{294462e3c265cf4a4e0df911f81c4de1fec26591},
        under \texttt{data/} (Table~\ref{tab:files}):
        \begin{itemize}
          \item \texttt{data/irexp\_commercial.jsonl.gz} (88,545): CC-BY-4.0 packaging,
                per-record CC-BY/CC0 stamps.
                This commercial DoR is also the Zenodo primary file
                \url{https://doi.org/10.5281/zenodo.22822285}.
          \item \texttt{data/irexp\_resolved\_commercial.jsonl.gz} (28,899):
                structure-linked commercial subset.
          \item \texttt{data/train\_no\_bench\_commercial.jsonl.gz} (28,753):
                commercial subset with benchmark InChIKeys held out.
        \end{itemize}
  \item Licence-bound companions on the same Hub revision, under \texttt{data/},
        excluded from the CC-BY dataset of record:
        \begin{itemize}
          \item \texttt{data/irexp\_sharealike.jsonl.gz} (1,897): Chemotion plus
                rare PMC ShareAlike; source licence CC-BY-SA-4.0.
          \item \texttt{data/irexp\_non\_commercial.jsonl.gz} (21,823): NC*.
        \end{itemize}
  \item Research-corpus accounting, not deposited as Hub files:
        the multi-licence total (121,233), ND (5), and empty/unknown (8,963),
        together with \texttt{seen\_papers.txt.gz} and
        \texttt{ir\_harvest\_snapshot.jsonl.gz}.
        Counts are in Table~\ref{tab:files}.
...
files unchanged).
That list is research-corpus tooling and is not a Hub file.
...
Section~\ref{sec:dataavail}) retains two flag-only fields that exist on that
pool only; rows are not dropped.
\texttt{ir\_shared\_in\_paper} is true for 18,651 rows;
...
\begin{itemize}
  ...
  \item \textbf{Licence filter.} The public dataset of record is the commercial
        file only (\texttt{data/irexp\_commercial.jsonl.gz}; CC-BY-4.0 packaging;
        per-record CC-BY/CC0 stamps; Section~\ref{sec:datarecords}).
        ShareAlike (1,897; \texttt{data/irexp\_sharealike.jsonl.gz}; source
        licence CC-BY-SA-4.0) and non-commercial (21,823;
        \texttt{data/irexp\_non\_commercial.jsonl.gz}; NC*) are separate Hub
        companions on the same revision.
        They remain bound by their source licences and are excluded from the
        CC-BY dataset of record.
        Do not relicence Chemotion or other ShareAlike rows as CC-BY.
        ND (5; CC-BY-ND), empty/unknown rows, and the multi-licence total
        \texttt{irexp.jsonl.gz} are research-corpus counts and are not Hub files.
        Attribute via \texttt{source\_doi} / \texttt{pmcid}.
        \texttt{source\_doi} may hold a PMC accession (\texttt{PMC:\ldots}) rather
        than a DOI.
  ...
  \item \textbf{Structure--NMR quarantine.} Before supervised training on
        the structure-linked research split, drop IDs in
        ...
        That list is retained with the research corpus and is not a Hub file.
  \item \textbf{Training without benchmark leakage.} If holding out structures used
        in a complementary elucidation benchmark, withhold those InChIKeys.
        \texttt{train\_no\_bench.jsonl.gz} is the multi-licence research split
        (rebuild with \texttt{scripts/build\_train\_no\_bench.py}).
        Commercial training uses
        \texttt{data/train\_no\_bench\_commercial.jsonl.gz} on the Hub revision
        (Section~\ref{sec:dataavail}).
  \item \textbf{Structure coverage.} Supervised structure tasks need rows with
        SMILES; 52.5\% of the research corpus lack SMILES.
        Use \texttt{data/irexp\_resolved\_commercial.jsonl.gz}
        (Section~\ref{sec:datarecords}).
  \item \textbf{FAIR reuse example.} For a redistributable training set, load the
        commercial DoR; drop IDs on the research-corpus quarantine list when
        that list is applied; keep \texttt{source\_doi} /
        \texttt{pmcid} for attribution; prefer structure-linked commercial rows
        when SMILES is required.
        Chemotion and other ShareAlike rows are in the public ShareAlike
        companion and may be remixed only under CC-BY-SA-4.0.
  \item \textbf{Attribution.} Cite this Data Descriptor and each record's
        \texttt{source\_doi} (PMC accession or DOI).
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Licence mix.} The research corpus is multi-licence.
        The public dataset of record is the commercial pool
        (Section~\ref{sec:datarecords}).
        ShareAlike and NC* companions are on the same Hub repository under
        their source licences and are excluded from that pool.
        ND and empty/unknown rows are not Hub files.
...
The commercial dataset of record (DoR) is the CC-BY/CC0 pool ($n=88{,}545$),
packaged under CC-BY-4.0, with per-record source licences still stamped
(Section~\ref{sec:datarecords}).
Hub revision \texttt{294462e3c265cf4a4e0df911f81c4de1fec26591} deposits that
commercial file with
\texttt{ir\_shared\_in\_paper} and \texttt{ir\_table\_flatten\_suspect}
retained on the commercial DoR only (rows not dropped), plus
\texttt{data/irexp\_resolved\_commercial.jsonl.gz} ($n=28{,}899$) and
\texttt{data/train\_no\_bench\_commercial.jsonl.gz} ($n=28{,}753$; a subset of
the structure-linked commercial file).
The same revision deposits
\texttt{data/irexp\_sharealike.jsonl.gz} (1,897; CC-BY-SA-4.0) and
\texttt{data/irexp\_non\_commercial.jsonl.gz} (21,823; NC*) as licence-bound
companions.
Those companions do not enlarge the CC-BY dataset of record.
Band lists on this revision include the thousands-separator repair defined in Methods.
The multi-licence research total is the construction reference in
Section~\ref{sec:datarecords} and is not a Hub file.
ND (5) and empty/unknown (8,963) rows are retained in the research corpus and
are not Hub files.
...
\begin{itemize}
  \item \textbf{Dataset of record}:
        \begin{itemize}
            \item Hugging Face Datasets, \url{https://huggingface.co/datasets/ilkhamfy/IRexp}, revision \texttt{294462e3c265cf4a4e0df911f81c4de1fec26591},
  ...
  \item \textbf{Harvest/pipeline source}: \url{https://github.com/IlkhamFY/spectro-agent}
   ...
   \item \textbf{License:}
    \begin{itemize}
          \item Commercial DoR (Hugging Face commercial file and the Zenodo primary file): CC-BY-4.0 packaging, with per-record source licences still stamped.
          \item ShareAlike companion on the same Hub revision (1,897; Chemotion 1,888 plus rare PMC ShareAlike): CC-BY-SA-4.0~\citep{chemotion2024}.
                This file is excluded from the CC-BY dataset of record.
          \item Non-commercial companion on the same Hub revision (21,823; NC*): excluded from the CC-BY dataset of record.
          \item ND (5; CC-BY-ND) is a research-corpus pool and is not a Hub file.
          \item PMC-sourced rows are mixed Creative Commons, stamped per article (\texttt{docs/LICENCE\_REMEDIATION.md}).
                Empty/unknown rows (8,963) and the multi-licence total are not Hub files.
          \item Only extracted numeric fields and identifiers are redistributed; source full texts are not.
    \end{itemize}
\end{itemize}
